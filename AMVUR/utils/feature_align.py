...
def feature_align(raw_feature: Tensor, P: Tensor, ori_size: tuple, device=None) -> Tensor:
    r"""
    ...
    for idx, feature in enumerate(raw_feature):
        ...
        _P = P[idx, :]
        interp_2d(feature, _P, ori_size, feat_size, out=F[idx, :])
    return F
...
def interp_2d(z: Tensor, P: Tensor, ori_size: Tensor, feat_size: Tensor, out=None, device=None) -> Tensor:
    ...
    P = (P - step / 2) / ori_size * feat_size
    out[:,:] = bilinear_interpolate_efficient(z,P)
...
def bilinear_interpolate_efficient(im: Tensor, P: Tensor, device=None):
    r"""
    Bi-linear interpolate 3d feature map to 2d coordinate (x, y).
    The coordinates are at the same scale of :math:`w\times h`.
    ...
    P_0 = torch.floor(P)
    P_1 = torch.floor(P_0) + 1
    P_0[:, 0] = torch.clamp(P_0[:, 0], 0, im.shape[2] - 1)
    P_1[:, 0] = torch.clamp(P_1[:, 0], 0, im.shape[2] - 1)
    P_0[:, 1] = torch.clamp(P_0[:, 1], 0, im.shape[1] - 1)
    P_1[:, 1] = torch.clamp(P_1[:, 1], 0, im.shape[1] - 1)

    P_0 = P_0.to(torch.int32)
    P_1 = P_1.to(torch.int32)

    Ia = im[:, P_0[:, 1].tolist(), P_0[:, 0].tolist()]
    Ib = im[:, P_1[:,1].tolist(), P_0[:, 0].tolist()]
    Ic = im[:, P_0[:, 1].tolist(), P_1[:, 0].tolist()]
    Id = im[:, P_1[:,1].tolist(), P_1[:,0].tolist()]

    # to perform nearest neighbor interpolation if out of bounds
    x_filter = P_0[:, 0] == P_1[:, 0]
    P_0[x_filter * (P_0[:, 0] == 0), 0] -= 1
    P_1[x_filter * (P_0[:, 0] != 0), 0] += 1
    y_filter = P_0[:, 1] == P_1[:, 1]
    P_0[y_filter * (P_0[:, 1] == 0), 1] -= 1
    P_1[y_filter * (P_0[:, 1] != 0), 1] += 1

    P_0 = P_0.to(torch.float32)
    P_1 = P_1.to(torch.float32)

    Wa = (P_1[:,0] - P[:, 0]) * (P_1[:, 1] - P[:, 1])
    Wb = (P_1[:, 0] - P[:, 0]) * (P[:, 1] - P_0[:, 1])
    Wc = (P[:, 0] - P_0[:, 0]) * (P_1[:, 1] - P[:, 1])
    Wd = (P[:, 0] - P_0[:, 0]) * (P[:, 1] - P_0[:, 1])
