...
def myimrotate(img, angle, center=None, scale=1.0, border_value=0, interpolation=cv2.INTER_LINEAR, auto_bound=False):
...
def myimresize(img, size, return_scale=False, interpolation=cv2.INTER_LINEAR):

    h, w = img.shape[:2]
    resized_img = cv2.resize(
        img, (size[0],size[1]), interpolation=interpolation)
    if not return_scale:
        return resized_img
    else:
        w_scale = size[0] / w
...
def transform(pt, center, scale, res, invert=0, rot=0):
    ...
    return new_pt[:2].astype(int)+1,t

def crop(img, center, scale, res, rot=0, interpolation=cv2.INTER_LINEAR):
    """Crop image according to the supplied bounding box."""
    # Upper left point
    ul = np.array(transform([1, 1], center, scale, res, invert=1)[0])-1
    # Bottom right point
    br = np.array(transform([res[0]+1, 
                             res[1]+1], center, scale, res, invert=1)[0])-1
    ...
    if not rot == 0:
        ...
        new_img = myimrotate(new_img, rot, interpolation=interpolation)
        new_img = new_img[pad:-pad, pad:-pad]

    # new_img = scipy.misc.imresize(new_img, res)
    new_img = myimresize(new_img, [res[0], res[1]],interpolation=interpolation)
    return new_img

def crop_bbshape(img, center, scale, res, rot=0, interpolation=cv2.INTER_LINEAR):
    """Crop image according to the supplied bounding box."""
    # Upper left point
    ul = np.array(transform([1, 1], center, scale, res, invert=1)[0])-1
    # Bottom right point
    ...
    bb_shape = new_img.shape[0:2]
    # new_img = scipy.misc.imresize(new_img, res)
    new_img = myimresize(new_img, [res[0], res[1]],interpolation=interpolation)
    return new_img, bb_shape
