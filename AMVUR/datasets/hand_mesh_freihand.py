...
from AMVUR.utils.image_ops import img_from_base64, crop, flip_img, flip_pose, flip_kp, transform, rot_aa
import torch
import torchvision.transforms as transforms
TRAIN_INDEX=1000
FREI_MAST_DIR = './datasets/FreiHAND_pub_v2/training/mask/'
...
class HandMeshTSVDataset(object):
    ...
    def j2d_processing(self, kp, center, scale, r, f):
        ...
        for i in range(nparts):
            kp[i,0:2] = transform(kp[i,0:2]+1, center, scale, 
                                  [self.img_res, self.img_res], rot=r)[0]
    ...
    def get_mask(self, image_key):
        cv2_mask = cv2.imread(FREI_MAST_DIR+image_key)
    ...
    def __getitem__(self, idx):
        ...
        has_mano = np.asarray(annotations['has_smpl'])
        ...
        # Process image
        img = self.rgb_processing(img, torch.tensor(center), torch.tensor(sc*scale), rot, flip, pn)
...
class HandMeshYamlDataset(HandMeshTSVDataset):
    ...
    def __init__(self, args, yaml_file, is_train=True, cv2_output=False, scale_factor=1):
        ...
        super(HandMeshYamlDataset, self).__init__(
