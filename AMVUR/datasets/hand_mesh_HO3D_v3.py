...
import time
import random
from AMVUR.utils.projection_utils import proj_func, batch_rodrigues, cam2pixel, pixel2cam, gen_trans_from_patch_cv, oriMesh2bbMesh, oriJoints2bbJoints
import matplotlib.pyplot as plt

from AMVUR.utils.tsv_file import TSVFile, CompositeTSVFile
from AMVUR.utils.tsv_file_ops import load_linelist_file, load_from_yaml_file, find_file_path_in_yaml
...
def cooridinate_openGl2openCV(coordinate):
    coord_change_mat = np.asarray([[1.,0.,0.],[0.,-1.,0.],[0.,0.,-1.]],dtype=np.float32)
    new_coordinate = np.matmul(coordinate, coord_change_mat.T)
...
class HandMeshTSVDataset(object):
    def __init__(self, args, data_dir, img_dir, depth_dir, meta_dir, txt_file, is_train=True, cv2_output=False, scale_factor=1):
        ...
        self.YCBModelsDir = './YCB_Video_Models'
        ...
        ###HO3D joints order
        self.HO3D_joints_definition = ('Wrist', 'Index_1', 'Index_2', 'Index_3', 'Middle_1', 'Middle_2', 'Middle_3', 'Pinky_1', 'Pinky_2', 'Pinky_3', 'Ring_1', 'Ring_2', 'Ring_3', 'Thumb_1', 'Thumb_2', 'Thumb_3', 'Thumb_4', 'Index_4', 'Middle_4', 'Ring_4', 'Pinky_4')
    ...
    def get_image(self, idx):
        file = self.file_list[idx].split('/')
        image_file = op.join(self.data_dir, file[0], self.img_dir, file[1]+'.jpg')
        cv2_im = cv2.imread(image_file)
        cv2_im = cv2.cvtColor(cv2_im, cv2.COLOR_BGR2RGB)
        return cv2_im

    def get_hand_center(self, meta):
        Ks = torch.Tensor(meta['camMat']).float()

        if 'handBoundingBox' in meta.keys():  # for evaluation set
            uv_hand = torch.tensor([[meta['handBoundingBox'][0], meta['handBoundingBox'][1]],
                                 [meta['handBoundingBox'][2], meta['handBoundingBox'][3]]])
        else:
            j3d = torch.Tensor(meta['handJoints3D']).float()
    ...
    def __getitem__(self, idx):
        ...
        meta['handJoints3D'] = cooridinate_openGl2openCV(meta['handJoints3D'])
        if self.is_train:
            mano_param = {
                'pose': meta['handPose'],
                'shape': meta['handBeta'],
            ...
            meta['handJoints3D'] = transform_joint_to_other_db(meta['handJoints3D'], self.HO3D_joints_definition,
        ...
        cam_param = {
                'focal': [meta['camMat'][0,0],meta['camMat'][1,1]],
                'princpt': [meta['camMat'][0,2],meta['camMat'][1,2]]
            }

        hand_center, scale, hand_uv, Ks = self.get_hand_center(meta)
        # mask = self.get_mask(img_key)
        if hand_center[0] < 650 and hand_center[1] < 490 and self.is_train:
            ...
            trans = np.asarray(mano_param['trans'])
        else:
        ...
        if self.is_train:
            ...
            root_coord = np.asarray(meta['handJoints3D'])
        ...
        joints_2d_transformed = self.j2d_processing(joints_2d.copy(), hand_center, sc*scale, rot, flip)
        ...
        meta_data['trans'] = torch.from_numpy(trans).float()
        meta_data['root_coord'] = torch.from_numpy(root_coord).float().unsqueeze(0)
        meta_data['rot'] = torch.tensor(rot).float()
