...
from AMVUR.utils.renderer import Renderer,visualize_reconstruction_ortho_proj
from AMVUR.utils.loss_function import keypoint_2d_loss,keypoint_3d_loss,vertices_loss,KLD_vertices_loss,KLD_3d_joints_loss,KLD_2d_joints_loss,tsa_pose_loss,pose_var_loss, photo_loss
...
def run(args, train_dataloader, METRO_model, mano_model, renderer, mesh_sampler):
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        # Transformer
        pred_camera, AMVUR_results, MANO_results, rendered_image = METRO_model(images, mano_model, renderer, ortho_proj=True, meta_masks=meta_masks, is_train=True)
        ...
        ###Mano
        pred_vertices_mano_samp,pred_vertices_mano_mu,pred_vertices_mano_var = MANO_results[1]
        pred_3d_joints_mano_samp, pred_3d_joints_mano_mu, pred_3d_joints_mano_var = MANO_results[0]

        pred_2d_joints_samp,pred_2d_joints_mu,pred_2d_joints_var = AMVUR_results[2]
        pred_2d_joints_mano_samp, pred_2d_joints_mano_mu, pred_2d_joints_mano_var = MANO_results[2]

        rendered_hand=rendered_image[0]
        mask=rendered_image[1]

        loss_vertices = vertices_loss(criterion_vertices, pred_vertices_samp, gt_vertices, has_mesh)
        loss_3d_joints = 0.5*keypoint_3d_loss(criterion_keypoints, pred_3d_joints_samp, gt_3d_joints_with_tag, has_3d_joints)+ \
                         0.5*keypoint_3d_loss(criterion_keypoints, pred_3d_joints_samp, gt_3d_joints_with_tag,
                                          has_3d_joints)
        loss_2d_joints = keypoint_2d_loss(criterion_2d_keypoints, pred_2d_joints_samp, gt_2d_joints, has_2d_joints)

        loss_reconstruction=loss_vertices+loss_3d_joints+loss_2d_joints

        loss_reconstruction_mano = keypoint_3d_loss(criterion_keypoints, pred_3d_joints_mano_samp, gt_3d_joints_with_tag, has_3d_joints)+ \
                    vertices_loss(criterion_vertices, pred_vertices_mano_samp, gt_vertices, has_mesh)+ \
                    keypoint_2d_loss(criterion_2d_keypoints, pred_2d_joints_mano_samp, gt_2d_joints, has_2d_joints)+ \
                    tsa_pose_loss(MANO_results[3])*0.02

        loss_KLD = KLD_2d_joints_loss(pred_2d_joints_mu,pred_2d_joints_var,pred_2d_joints_mano_mu,pred_2d_joints_mano_var)+\
                   KLD_3d_joints_loss(pred_3d_joints_mu,pred_3d_joints_var,pred_3d_joints_mano_mu,pred_3d_joints_mano_var)+\
        ...
        loss = loss_reconstruction + loss_KLD*0.1 + loss_reconstruction_mano + loss_texture*0.001
        ...
        if iteration % args.logging_steps == 0 or iteration == max_iter:
            ...
            logger.info(
                ...
                + '  loss: {:.4f}, 2d joint loss: {:.4f}, 3d joint loss: {:.4f}, vertex loss: {:.4f}, reconstruction_loss: {:.4f}, KLD_loss:{:.4f}, texture_loss:{:.4f},compute: {:.4f}, data: {:.4f}, lr: {:.6f}'.format(
                    log_losses.avg, log_loss_2djoints.avg, log_loss_3djoints.avg, log_loss_vertices.avg, log_loss_reconstruction.avg,log_loss_KLD.avg,log_loss_texture.avg, batch_time.avg, data_time.avg,
...
def run_inference_hand_mesh(args, val_loader, METRO_model, criterion, criterion_vertices, epoch, mano_model, mesh_sampler, renderer):
    ...
    with torch.no_grad():
        for i, (img_keys, images, annotations) in enumerate(val_loader):
            ...
            # forward-pass
            pred_camera, AMVUR_results, MANO_results, rendered_images = METRO_model(images, mano_model, renderer, ortho_proj=True)
            rendered_hand = rendered_images[0]
            mask = rendered_images[1]
            # obtain 3d joints from full mesh
            gt_root_joint_cam = annotations['root_coord'].to(AMVUR_results[1][1].device)
            pred_3d_joints_from_mesh = mano_model.get_3d_joints(AMVUR_results[1][1])
            ...
            pred_vertices_output = AMVUR_results[1][1] - pred_3d_wrist[:, None, :]+gt_root_joint_cam
            HO3D_joints_name = (
...
def visualize_mesh( renderer,
    ...
    for i in range(min(batch_size, 10)):
        ...
        rend_img = visualize_reconstruction_ortho_proj(img, 224, gt_keypoints_2d_, vertices, pred_keypoints_2d_, cam, renderer,mano_face)
